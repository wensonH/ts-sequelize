...
export class OracleQuery extends AbstractQuery {
  ...
  public _run(connection, sql : string, parameters : {}) {
    ...
    if (_.startsWith(self.sql, 'BEGIN TRANSACTION')) {
      ...
      return connection.execute(self.sql, Object.keys(self.outParameters).length > 0 ? self.outParameters : [], { outFormat: self.outFormat, autoCommit : self.autoCommit }).then(result => {
  ...
  public run(sql : string, parameters : {}) {
    const self = this;
   ...
   * @hidden
   */
  private _replaceLongAliases(tableAliases : any, result : any) : any {
   ...
   */
  public _dealWithLongAliasesBeforeSelect(sql : string) : { sql? : string, tableAliases?} {
   ...
   * @hidden
   */
  private handleShowConstraintsQuery(data) {
   ...
   * @hidden
   */
  private convertStringToObj(path : string, value : any, obj : {}) : void {
   ...
   * @hidden
   */
  private isSelectCountQuery() : boolean {
   ...
   * @hidden
   */
  private handleShowIndexesQuery(data : any[]) : any[] {
    const acc = [];
